...
def record_payment_transaction(user_id, bill_id, amount):
    """
    Handles the payment transaction:
    1. Updates the bill status to 'Paid'.
    2. Inserts a new record into the payments table.
    Both must succeed as a single transaction (ACID).
    """
    conn = create_connection()
    if conn is None:
    ...
    try:
        ...
        update_bill_sql = """
            ...
            WHERE bill_id = ? AND user_id = ? AND status = 'Pending';
        """
        cursor.execute(update_bill_sql, (bill_id, user_id))
        
        # Check if exactly one bill was updated (prevents double-payment or wrong user)
        if cursor.rowcount != 1:
            conn.rollback()
            return False, "Bill not found, already paid, or invalid user ID."

        # 2. Insert the payment record
        insert_payment_sql = """
            INSERT INTO payments (user_id, bill_id, amount, payment_date, payment_method)
            VALUES (?, ?, ?, CURRENT_TIMESTAMP, 'Online Payment');
        """
        cursor.execute(insert_payment_sql, (user_id, bill_id, amount))
        ...
        conn.rollback()
        print(f"Database transaction error during payment: {e}")
        return False, f"Payment failed due to internal error."
